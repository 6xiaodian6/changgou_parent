 ...
 * @description:
 * @author: yuandian
 * @createTime: 2019-11-17 20:40:27
 **/
@Service
public class SearchServiceImpl implements SearchService {
    ...
    public Map search(Map<String, String> searchMap) throws Exception {
        ...
        //有条件才进行查询
        if (null != searchMap) {
            //组合条件查询，也就是多个条件的查询进行与或非的条件组合全由这个对象进行设置
            ...
            searchMap.keySet().forEach(key -> {
                if (key.startsWith("spec_")) {
                    ...
                    boolQuery.filter(QueryBuilders.termsQuery("specMap." + key.substring(5) + ".keyword", value));
                }
            });
            //3.条件查询，价格条件
    ...
    /**
     * 商品属性及规格显示，也就是将字符串形式的规格转换为对象，也就是原有的是list集合转换为map集合
     * /**
     * 原有数据
     * [
